...
# Global variables
api_key = api_key = os.getenv("BRICKSET_KEY")
...
def _get_hash() -> str:
    ...
    try:
        resp = requests.get(url + "/login", params=query, timeout=12)
...
def _get_themes() -> str:
    ...
    try:
        resp = requests.get(url + "/getThemes", params=query, timeout=12)
        resp.raise_for_status()
        data = resp.json()
        ...
        return json.dumps({"error": f"Error calling Brickset getThemes"})
        

def _get_sets(getID=False ,**extra) -> str:
    ...
    try:
        resp = requests.get(url + "/getSets", params=query, timeout=12)
        resp.raise_for_status()
        data = resp.json()
        ...
        return json.dumps({"error": f"Error calling Brickset getSets: {e}"})

def _get_reviews(setNumber) -> str:
    query = {
        "apiKey": api_key,
        "setID": _get_sets("true", setNumber=setNumber)   
    }

    try:
        resp = requests.get(url + "/getReviews", params=query, timeout=12)
        resp.raise_for_status()
        data = resp.json()
        ...
        return json.dumps({"error": f"Error calling Brickset getReviews: {e}"})
...
class GetReviewsInput(BaseModel):
    ...
    setNumber: Optional[str] = Field(default=None, description="e.g. 75192-1 (auto-fetches setID required for this method)")
...
brickset_get_reviews_tool = StructuredTool.from_function(
    func=lambda **kwarg: _get_reviews(**kwarg),
...
brickset_get_themes_tool = StructuredTool.from_function(
    func=lambda **kwargs: _get_themes(**kwargs),
