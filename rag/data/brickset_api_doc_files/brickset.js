$(function () {
    ...
    setupAddTagForm()
    setupHighslide();
    setupOwnershipControls();
    setupReviewRatings();
    setupBricklistRatings();
    setupVideoReviews();
    setupReviewAdmin();
    setupBrickListAdmin();
    setupQueryAdmin();
   ...
   // setupPartsAdmin();
    ...
    setupResultsPage("aside.parts", "parts");
...
function setupDarkMode() {
    ...
    var darkModeOverrideCookie = "darkmodeoverride"
    var lightTitle = 'Switch to light mode';
    var darkTitle = 'Switch to dark mode'

    if ($.cookie(darkModeOverrideCookie) != 'override') {
        if (window.matchMedia('(prefers-color-scheme: dark)').matches) {
            $.cookie(cookieName, 'on', { expires: 365, path: '/' });
        }
    }

    if ($.cookie(cookieName) == 'on') {
        $('body').addClass('dark');
        $('#darkmodeswitch').text('Light').attr('title', lightTitle);
    } else {
        $('#darkmodeswitch').text('Dark').attr('title', darkTitle);
    }
    ...
    $('#darkmodeswitch').click(function (e) {
        ...
        if ($.cookie(cookieName) == 'on') {
            ...
            $('#darkmodeswitch').text('Dark').attr('title', darkTitle);
            $.cookie(cookieName, 'off', { expires: 365, path: '/' });
            $.cookie(darkModeOverrideCookie, 'override', { expires: 1, path: '/' });
        } else {
            $('body').addClass('dark');
            $('#darkmodeswitch').text('Light').attr('title', lightTitle);
            $.cookie(cookieName, 'on', { expires: 365, path: '/' });
...
function setupLinkPopups() {

    $("a[href *= '/sets/'], a[href *= '/parts/'], a[href *= '/minifigs/'], a.amazonlink").hoverIntent({
        over: function () {
        //    if ($(this).attr("href").includes("images.") == false) { 
                $(this).tooltip({
                    content: function (response) {
                        
                        if (this.innerHTML.includes('<span>') == false) {
                            $.ajax({
                                url: "/ajax/getDetailsForPopup",
                                data: {
                                    'url': $(this).attr("href")
                                },
                                type: "POST"
...
function setupVariousThings() {

    $(".neattable tr:even").addClass("alt");

    $("img").on("error", function () { // not working??
        $(this).attr("src", "https://brickset.com/assets/images/spacer2.png");
    });
    ...
    if (typeof articleID != 'undefined') {
        $("#viewCount").load('/ajax/news/updateViewCount?articleID=' + articleID)
    }


    if (typeof brickLinkSet != 'undefined') {
        $("#brickLinkOutput").load('/ajax/bricklink?t=set&q=' + brickLinkSet + "&cc=" + brickLinkCC)
    }

    if (typeof brickLinkMinifig != 'undefined') {
        $("#brickLinkOutput").load('/ajax/bricklink?t=minifig&q=' + brickLinkMinifig +"&cc=" + brickLinkCC)
    }

    if (typeof brickLinkPart != 'undefined') {
        $("#brickLinkOutput").load('/ajax/bricklink?t=part&q=' + brickLinkPart + "&cc=" + brickLinkCC)
    }

    if (typeof stockXSet != 'undefined') {
        $("#stockXOutput").load('/ajax/sets/stockx?s=' + stockXSet + "&cc=" + stockXCC)
    }

    if (typeof wickedBrickSet != 'undefined') {
        $("#wickedBrickOutput").load('/ajax/sets/wickedBrick?s=' + wickedBrickSet)
    }

    if (typeof doEbaySearch != 'undefined') {
        ebaySearch()
    }
    if (typeof rebrickableSet != 'undefined') {
        $("#rebrickableOutput").load('/ajax/sets/rebrickable?set=' + rebrickableSet);
    }

    if (typeof shopLEGOcomSet != 'undefined') {
        $("#shopLEGOComOutput").load('/ajax/sets/shopLEGOcom?id=' + shopLEGOcomSet);
    }

    if (typeof alternateBuildCountSetNumber != 'undefined') {
        $("#alternateBuildCount").load('/ajax/sets/altBuildCount?set=' + alternateBuildCountSetNumber);
    }

    //if (typeof swooshableSetNumber != 'undefined') {
    //    var currentCount = $("#swooshableCount").text();
    //    $("#swooshableCount").load('/ajax/sets/swooshableCount?c=' + currentCount + '&set=' + swooshableSetNumber);
    //}

    if (typeof drawCollectionGraph_theme == 'function') {
        drawCollectionGraph_theme();
    }

    if (typeof drawCollectionGraph_subtheme == 'function') {
        drawCollectionGraph_subtheme();
    }

    if (typeof drawCollectionGraph_year == 'function') {
        drawCollectionGraph_year();
    }

    if (typeof drawCollectionGraph == 'function') {
        drawCollectionGraph();
    }

    if (typeof drawCollectionGraph_category == 'function') {
        drawCollectionGraph_category();
    }

    if (typeof disableCollectionControls != 'undefined') {
        $('.ownCheckbox').attr('disabled', 'disabled');
        $('.wantCheckbox').attr('disabled', 'disabled');
        $('.flagCheckbox').attr('disabled', 'disabled');
        $('.minifigFlagCheckbox').attr('disabled', 'disabled');
        $('.qtyOwned').attr('disabled', 'disabled');
        $('.qtyOwnedNew').attr('disabled', 'disabled');
        $('.qtyOwnedUsed').attr('disabled', 'disabled');
        $('.qtyWanted').attr('disabled', 'disabled');
        $('.wantedPriority').attr('disabled', 'disabled');
        $('.acmlink').hide();
    }
 ...
 //   $("img.thumbnail").lazyload();
    $('div.featurebox ul.tools').nmcDropDown({});
    ...
    $('div.browselinks select').selectBoxIt();
    ...
    $('div.browselinks select').change(function (e) {
        if ($(this).val())
            location.replace($(this).val())
    });


    // Drag/drop file upload from https://www.codeguru.com/csharp/csharp/cs_internet/uploading-files-using-html5-drag-and-drop-and-asp.net.htm
    var box = document.getElementById("fileuploadbox");
    if (box != null) {
        ...
        function OnDrop(e) {
            ...
            $.ajax({
                type: "POST",
                url: "/webhandlers/fileUpload.ashx?m=" + $("#fileuploadmessage").length + "&id=" + articleID,
                contentType: false,
                processData: false,
                data: data,
                success: function (result) {
                    if ($("#fileuploadmessage").length) { // image upload page
                        $("#fileuploadmessage").html(result)
                    } else { // article edit page

                        var data = result.split(';', 2);
                        var thumb = data[0];
                        var original = data[1];

                        $('#ctl00_mainContent_Repeater1_ctl00_ogImageURL').val(original)
                        $('#ctl00_mainContent_Repeater1_ctl00_mainImageURL').val(thumb)
                        $('#ctl00_mainContent_Repeater1_ctl00_mainImageNavigateURL').val(original)
                ...
                error: function () {
                    $("#fileuploadmessage").text("Error uploading files. Please try again.");
                }
            });
        }
    }
    ...
    var box = document.getElementById("fileuploadbox2");
    if (box != null) {
        box.addEventListener("dragenter", OnDragEnter, false);
        box.addEventListener("dragover", OnDragOver, false);
        box.addEventListener("drop", OnDrop, false);
        ...
        function OnDrop(e) {
            ...
            $.ajax({
                type: "POST",
                url: "/webhandlers/fileUpload.ashx?m=2&id=" + articleID, 
                contentType: false,
                processData: false,
                data: data,
                success: function (result) {
                    $("#fileuploadmessage2").html(result)
                },
                error: function () {
                    $("#fileuploadmessage2").text("Error uploading files. Please try again.");
                }
            });
        }
    }
    ...
    $('#showimages').click(function (e) {
        e.stopPropagation();
        e.preventDefault();
        var data = this.href.split(';', 2);
        var articleID = data[1];
        $("#fileuploadmessage2").html('Retrieving images...')
        $.post('/webhandlers/getnewsimages.ashx', { 'id': articleID}, function (data) {
            $("#fileuploadmessage2").html(data);
        });
    });



    box = document.getElementById("imageuploadbox");
    if (box != null) {
        box.addEventListener("dragenter", OnDragEnter, false);
        box.addEventListener("dragover", OnDragOver, false);
        box.addEventListener("drop", OnDrop, false);
        ...
        function OnDrop(e) {
            ...
            $.ajax({
                type: "POST",
                url: "/webhandlers/setImageUpload.ashx?s=" + setNumber + "&t=" + imageType,
                contentType: false,
                processData: false,
                data: data,
                success: function (result) {
                    if (result != '') {
                        ...
                        $("#fileuploadmessage").text("Error uploading file. Please try again.");
    ...
    $(".tagsfilter .remove").show();


    /* tag admin page */
    var match = window.location.href.split('?')[1];
    $('#tagTheme').find('option[value$="' + match + '"]').attr('selected', true);

    $('#tagTheme').change(function (e) {
    ...
    $(".smlinks img").hover(
        function () { this.src = this.src.replace("_m", "_c"); },
        function () { this.src = this.src.replace("_c", "_m");
        });

    $('input:radio[name=which]').change(function () {
        locSnip = window.location.href.split('?')[0];
        window.location.href = locSnip + "?query=" + $(this).val()
    });

    if (setup == 1) {
        $.post('/ajax/master/setup', {}, function (data) { $("#userstats").replaceWith(data); });
    }


    //$('.deleteparttag').click(function (e) {
    //    e.stopPropagation();
    //    e.preventDefault();
    //    var data = id.split(';', 2);
    //    var id = data[0];
    ...
    //    $.ajax({
    //        url: "/WebHandlers/delPartTag.ashx",
    //        data: "id=" + id + "&userID=" + userid,
    //        type: "POST",
    //        async: false,
    //        success: function (data) { $('#deltag' + id).slideUp(); },
    //        error: function () { }
    //    });
    //});

    $('input.partsPlatform').click(function () {
        var data = this.value.split(';', 2);
        var DesignID = data[0];
        var Platform = data[1];
        $.post('/webhandlers/setPartPlatform.ashx', { 'DesignID': DesignID, 'Platform': Platform }, function (data) { });
    });


    $('a.blockCommentsLink').click(function (e) {
        e.stopPropagation();
        e.preventDefault();
        var data = this.href.split(';', 3);
        var UserID = data[1];
        var BlockUserName = data[2];
        $.post('/webhandlers/blockUserComments.ashx', { 'UserID': UserID, 'BlockUserName': BlockUserName }, function (data) {
            $("#blockCommentsOutput").replaceWith(data);
        });
    });

    $('a.reinstateCommentsLink').click(function (e) {
        e.stopPropagation();
        e.preventDefault();
        var data = this.href.split(';', 3);
        var UserID = data[1];
        var BlockUserID = data[2];
        $.post('/webhandlers/reinstateUserComments.ashx', { 'UserID': UserID, 'BlockUserID': BlockUserID }, function (data) {
            $("#reinstateCommentsOutput").replaceWith(data);
        });
    });
    ...
    $('input.asininput').pressEnter(function (e) {
        var data = this.id.split('_', 2);
        var region = data[0];
        var setID = data[1];
        $('#s_' + this.id).html(this.value);
        $.post('/webhandlers/setasin.ashx', { 'Region': region, 'SetID': setID, 'ASIN': this.value }, function (data) {
...
function setupPoll() {
    ...
    if (typeof checkboxLimit != 'undefined') {
        $('input[type = checkbox]').on('change', function (evt) {
            if ($(this).siblings(':checked').length >= checkboxLimit) {
                this.checked = false;
            }
        });
    }

    $('.pollsubmit').click(function (e) {
        e.stopPropagation();
        e.preventDefault();

        var id = right(this.id, this.id.length - 1);

        // Radio buttons
        var option = $('input[name=poll' + id + ']:checked').val();

        if (option != undefined) {
            savePollResponse(id, option);
        }

        // Checkboxes
        $('input[type=checkbox]').each(function () {
            if (this.checked) {
                savePollResponse(id, $(this).val());
            }
        });
    });

    $('.pollresults').click(function (e) {
        e.stopPropagation();
        e.preventDefault();

        var id = right(this.id, this.id.length - 1);
        $.post('/webhandlers/getPollResponses.ashx', { 'id': id }, function (data) {
            $("#message" + id).html(data);
        });
    });
}

function savePollResponse(id,option) {

    $.post('/webhandlers/savePollResponse.ashx', { 'id': id, 'option': option }, function (data) {
        $("#s" + id).hide();
        $("#option" + id).hide();
        $("#message" + id).html(data);
        $.cookie('poll' + id, 1, { expires: 30 });
        setupPoll();
    });
}
...
function setupSliders() {
    $('body.faq h2 a').click(function (e) {
        e.stopPropagation();
        e.preventDefault();
    ...
    $('.viewoptions a.options').click(function (e) {
        e.stopPropagation();
        e.preventDefault();
        if (!$(this).parent().next().is(":visible")) {
            $(this).parent().next().slideDown();
...
function setupLikeButtons() {
    $('a.articlelikebutton').click(function (e) {
        e.stopPropagation();
        e.preventDefault();

        var id = right(this.id, this.id.length - 4);
        var data = id.split(';', 2);
        var articleid = data[0];
        var userid = data[1];
        var elem = $(this)

        var src = $("#img" + articleid).attr('src');

        if (src.indexOf("normal") > 0) {
            src = src.replace("normal", "highlight");
            $.post('/webHandlers/articleLike.ashx', { 'userid': userid, 'articleid': articleid, 'like': 1 }, function (data) {
                $("#liketext" + articleid).html(data)
                elem.parent().addClass("liked").removeClass("notliked");
            });
        } else {
            src = src.replace("highlight", "normal");
            $.post('/webHandlers/articleLike.ashx', { 'userid': userid, 'articleid': articleid, 'like': 0 }, function (data) {
                $("#liketext" + articleid).html(data)
                elem.parent().addClass("notliked").removeClass("liked");
            });
        }

        $("#img" + articleid).attr('src',src);

    });

    $('a.commentlikebutton').click(function (e) {
        e.stopPropagation();
        e.preventDefault();
        ...
        var userid = data[1];
        var src = $("#img" + commentid).attr('src');
        var elem = $(this)

        if (src.indexOf("normal") > 0) {
            src = src.replace("normal", "highlight");
            $.post('/webHandlers/commentLike.ashx', { 'userid': userid, 'commentid': commentid, 'like': 1 }, function (data) {
                $("#liketext" + commentid).html(data)
                elem.parent().addClass("liked").removeClass("notliked");
            });
        } else {
            src = src.replace("highlight", "normal");
            $.post('/webHandlers/commentLike.ashx', { 'userid': userid, 'commentid': commentid, 'like': 0 }, function (data) {
                $("#liketext" + commentid).html(data)
                elem.parent().addClass("notliked").removeClass("liked");
            });
        }
...
function setupContactPage() {
    $('body.contact dt a').click(function (e) {
        e.stopPropagation();
        e.preventDefault();
        $('dd').slideUp();
        if (! $(this).parent().next().is(":visible"))
    ...
    $('a.showformTrue').click(function (e) {
        e.stopPropagation();
        e.preventDefault();
    ...
    $('a.showformFalse').click(function (e) {
        e.stopPropagation();
        e.preventDefault();
...
function setupTabs() {
    ...
    var msg = "<div class='tab-loading textcenter'><img src='/assets/images/misc/ajax-loader-large.gif' /></div>";
    var tabOpts1 = {
        cache: false,
        ajaxOptions: { cache: false },
        load: function (e, ui) { $(ui.panel).find(".tab-loading").remove(); },
        select: function (e, ui) { var $panel = $(ui.panel); if ($panel.is(":empty")) { $panel.append(msg) } },
        activate: function (event, ui) {
            $.cookie(this.id, ui.newTab.index()); 
        }
     };
    ...
    var tabOpts2 = {
        ...
        load: function (e, ui) { $(ui.panel).find(".tab-loading").remove(); },
        select: function (e, ui) { var $panel = $(ui.panel); if ($panel.is(":empty")) { $panel.append(msg) } },
        active: $.cookie($('body').attr('class').trim()),
        activate: function (event, ui) {
            $.cookie($('body').attr('class').trim(), ui.newTab.index(), { expires: 1 });
        }
    };
    ...
    $(function () { $("body.sets .tabs, body.minifigs .tabs, body.parts .tabs").tabs(tabOpts1).show(); }); 
    $(function () { $("body.default .tabs, body.article .tabs").tabs(tabOpts2).show(); });

    $(function () { $("body.administration .tabs").tabs(tabOpts3).show(); }); 
    $(function () { $(".tabs.aside ul.tablist").show() });
    

    if (typeof activeTopTab != 'undefined')
        $(function () { $("#settabtop").tabs("option", "active", activeTopTab) });
    else
        $(function () { $("#settabtop").tabs("option", "active", $.cookie('settabtop')) })


    $(function () { $("#settabbottom").tabs("option", "active", $.cookie('settabbottom')) })
...
function setupAddToMenu() {
    $('a.addtomenu').click(function (e) {
        e.stopPropagation();
        e.preventDefault();
        var userID = right(this.id, this.id.length - 1)
        var link = location.href
        var title = document.title
        $.post('/webhandlers/addtomenu.ashx', { 'link': link, 'title': title, 'userID': userID }, function (data) {
            $("#userlinks").hide().html(data).fadeIn(500)
            
        });
        var origHTML = $(this).html();
        $(this).html($(this).html() + '<span> -- Added!</span>');
        $('a.addtomenu span').fadeOut(1000);
        $(this).html(origHTML);
    });
}
...
function setupBuyPage() {
    $('#ebaySearch').click(function (e) {
        e.preventDefault();
        ebaySearch();
    });

    $('a.hideamazontheme').click(function (e) {
        e.preventDefault();
    ...
    var themeList = $.cookie('amazonThemes3')
    var themes

    if (themeList != null) {
        $(".amazonblock").hide();
        
        themes = themeList.split(",");
        $('#selectAll').prop('checked', false);

        for (var i in themes) {
            if (themes[i] != '') {
                $('#cb_' + themes[i]).prop('checked',true);
                $('#div_' + themes[i]).slideDown()
            }
        };

        if (themes.length - 1 == $('.themeCheckbox').length)
    ...
    $('.themeCheckbox').click(function (e) {
        saveAmazonCookie();
        if (this.checked)
            $('#div_' + this.value).slideDown()
        else
            $('#div_' + this.value).slideUp();
    });

    $('.countryCheckbox').click(function (e) {
        saveAmazonCountryCookie();
        if (this.checked)
    ...
    var countryList = $.cookie('amazonCountries')
    if (countryList != null) {
        var countries = countryList.split(",");

        for (var i in countries) {
            if (countries[i] != '') {
                $('#ccb_' + countries[i]).prop('checked', true);
                $("." + countries[i].toLowerCase()).show()
            }
        };
    } else {
        $(".us").show();
        ...
        $(".de").show();
        $(".nl").show();
        $('#ccb_UK').prop('checked', true);
        $('#ccb_US').prop('checked', true);
        $('#ccb_DE').prop('checked', true);
        $('#ccb_NL').prop('checked', true);

        if (typeof country != 'undefined') {
            $("." + country.toLowerCase()).show();
            $('#ccb_' + country).prop('checked', true);
        }
    }
...
function saveAmazonCookie2() {
    var cookie = '';
    $.cookie('amazonThemes2', cookie, { expires: 365 });
    $('a.hideamazontheme').each(function () {
        if ($(this).html() == 'Show') {
...
function saveAmazonCookie() {
    var cookie = '';
    $.cookie('amazonThemes3', cookie, { expires: 365 });
    $('.themeCheckbox').each(function () {
        if (this.checked)
...
function saveAmazonCountryCookie() {
    var cookie = '';
   $.cookie('amazonCountries', cookie, { expires: 365 });
...
function ebaySearch() {
    //var loading = '<center><img src="/assets/images/misc/ajax-loader-large.gif" /></center>'
    //var site = $('#ebaySite').val();
    //var query = $('#ebayQuery').val();
    //if (query != '') {
    //    $('#ebayListContainer').html(loading);
    //    $.get('/ajax/sets/ebay.aspx', { 'site': site, 'query': query }, function (data) { $('#ebayListContainer').html(data); });
    //}
}
...
function setupArticlePage() {
    ...
    $('#commentsform').submit(function (e) {
        ...
        $.post('/ajax/news/addComment', { 'comment': comment, 'userid': userid, 'articleid': articleid }, function (data) {
    ...
    var interval = setInterval(function () {
        ...
        if (minutes < 0) {
            ...
            $('.countdown').html('Your session has timed out. Please cut your comment, refresh the page, then paste it in again.');
            return;
        }
        ...
        $('.countdown').html('Your session will time out in ' + timer2 + '.');
...
function setupCommentAdmin() {

    $('section.comments a.reportcomment').click(function (e) {
        e.preventDefault();
        var id = right(this.id, this.id.length - 1);
        $('#ajaxContainer').dialog("option", "position", { my: "center", at: "center", of: window });
        $('#ajaxContainer').empty().load("/ajax/news/reportComment?commentID=" + id).dialog("open");
    ...
    comment.on("focus", function () { 
        comment.putCursorAtEnd()
    });

    $('section.comments a.replycomment').click(function (e) {
        e.preventDefault();
        e.stopPropagation();
        var id = right(this.id, this.id.length - 1);
        $.post('/webHandlers/getComment.ashx', { 'commentID': id}, function (data) {
            $("#comment").html(data);
            $("#comment").focus();
    ...
    $('section.comments a.editcomment').click(function (e) {
        $('section.comments a.editcomment').unbind('click');
        e.preventDefault();
        e.stopPropagation();
        var id = right(this.id, this.id.length - 1)
        var t = $("#comment" + id + " .comment p").html();

        t = t.replace(/<br>/g, "&#10;");
        t = t.replace(/<i>\"/g, "[[");
        t = t.replace(/\"<\/i>/g, "]]");

        $("#comment" + id + " .comment p").html("<textarea id='ta" + id + "'>" + t + "</textarea><input type='button' value='Save' class='commentSave' name='" + id + "'/>");
        $("#comment" + id + " .action").html("<a href='#' class='commentCancelEdit' id='c" + id + "' >Cancel</a>&nbsp;");
        $("#comment" + id + " .action").addClass("cancel");

        $("#comment" + id + " .links").hide();

        $('input.commentSave').click(function () {
            var commentID = this.name;
            var comments = $('#ta' + this.name).val();
            $.post('/webHandlers/updateComment.ashx', { 'commentID': commentID, 'comments': comments }, function (data) {
                $("#comment" + commentID).html(data);
                $("#comment" + commentID + " .links").hide();
                setupCommentAdmin();
            });
            return false;
        });

        $('a.commentCancelEdit').click(function () {
            e.preventDefault();
            e.stopPropagation();
            var commentID = right(this.id, this.id.length - 1);
            $.post('/webHandlers/updateComment.ashx', { 'commentID': commentID, 'comments': '' }, function (data) {
                $("#comment" + commentID).html(data);
               $("#comment" + commentID + " .links").hide();
                setupCommentAdmin();
            });
    ...
    $('section.comments a.delcomment').click(function (e) {
        e.preventDefault();
        e.stopPropagation();
        if (window.confirm("Click OK confirm you want to delete this comment.")) {
            var id = right(this.id, this.id.length - 1)
            $.post('/WebHandlers/delComment.ashx', { 'commentID': id }, function (data) {
                $("#comment" + id).fadeOut();
            });
        }
        return false;
...
function setupResultsPage(selector, cookiePrefix) {

    $(selector + ' div.addtags select').change(function (e) {
        if ($(this).val())
            location.replace($(this).val())
    });

    $(selector + ' div.tags select').change(function (e) {
        if ($(this).val())
            location.replace($(this).val())
    });

    $(selector + ' div.resultsfilter ul.pagelength li a').click(function (e) {
       e.preventDefault();
       $.cookie(cookiePrefix + 'PageLength', $(this).text(), { expires: 365, path: '/' });
    ...
    $(selector + ' #sortby').change(function (e) {
        $.cookie(cookiePrefix + 'SortOrder', $(this).val(), { expires: 365, path: '/' });
        var url = window.location.href;
        url = url.replace(/\/order-[a-zA-Z]*/, "");
        location.replace(url);
    });

    if (cookiePrefix == 'sets' || cookiePrefix == 'minifigs' || cookiePrefix == 'parts') {
        $(selector + ' div.resultsfilter ul.viewswitch li a').click(function (e) {
            $.cookie(cookiePrefix + 'ListFormat', $(this).text(), { expires: 365, path: '/' });
            e.preventDefault();

            //if ($('body').attr('class').includes('designview')) { // for parts list, designview also has galleryview class
            //    location.replace(window.location.href);
            //}

            if ($('body').attr('class').includes('listview') || $('body').attr('class').includes('galleryview')) {

                $('.resultsfilter .viewswitch li').removeClass('active');
                $(this).parent().addClass('active');

                if ($(this).text().toLowerCase().indexOf("list") != -1) {
                    $('body').removeClass('galleryview').addClass('listview');
                //    $('article.set, article.set > .mainimg, article.set .meta .tags').css('height', 'auto');

                } else if ($(this).text().toLowerCase().indexOf("gallery") != -1) {
                    $('body').removeClass('listview').addClass('galleryview');
                    //equalheight('article.set > .mainimg');
                    //equalheight('article.set .meta .tags');
                    //equalheight('article.set');
                } else {
                    location.replace(window.location.href);
                }
            } else {
...
var notLoggedIn = 'You must be logged in to be able to use this feature';
...
function contactForm(ev) {
    ...
    $.post('/WebHandlers/contactForm.ashx', { 'name': name, 'email': email, 'message': message}, function (data) {
        showMessage("#contactFormResult", data);
...
//    $.post('/WebHandlers/signupForm.ashx', { 'name': name, 'email': email }, function (data) {
//        showMessage("#signupFormResult", data);
//        $('#signupSubmit').show();
//    });
//    return false;
//}

function extLinks(selector) {
    $(selector + ' a').each(function () {
        try {

            var a = new RegExp('brickset.*com|javascript|localhost|/MN/|lego.com/cdn|lego.com/service|catawiki|staticflickr|instagram'); // --> /MN/ = bricklink minifig image,
            var b = new RegExp('/click/|twitter');
            if (this.href != '') {
                if (!a.test(this.href) || b.test(this.href)) {
                    ...
                    $(this).attr('title', title + 'Link to external website, opens in a new tab.');
...
function autocomplete() {
    $("#searchQuery").autocomplete({
        source: function (request, response) {
            $.ajax({
                ...
                data: "d=" + $('#searchScope').val() + "&q=" + request.term.trim(),
                dataType: "json",
                type: "GET",
                contentType: "application/json; charset=utf-8",
                dataFilter: function (data) { return data; },
                success: function (data) { response(data.d.split(",")); },
                error: function () { }
            });
        },
        minLength: 3,
        focus: function (event, ui) { search_option = ui.item; },
        select: function (event, ui) {
            $.post('/ajax/master/recordSearch', { 'query': $("#searchQuery").val(), 'selected': ui.item.value }, function (data) { });
            $("#searchQuery").val(ui.item.value); $("#searchForm").submit()
        },
    })
    ...
    if ((scope == 'Parts') || (scope == 'Minifigs') || (scope == 'Sets') || (scope == 'All'))
        $("#searchQuery").autocomplete("option", "disabled", false);
    else
...
function setupViewMoreLinks() {
    $('#viewmorenews').click(function (e) {
        e.preventDefault();
        $.get("/ajax/homepage/news", function (data) {
            var r = Math.random();
            $('#news').append($('<div id="div' + r + '">' + data + '</div>').hide().fadeIn(500));
            document.getElementById("div" + r).scrollIntoView();
        });
        return false;
    });
}
...
function setupHomePageShopping() {

    if (typeof vendor != 'undefined') {
        $('.content .shopping .retailer').load("/ajax/shopping/header?vendor=" + vendor);
        $('.content .shopping .retailernav li.' + vendor).addClass('active');
    }

    $('.content .shopping .retailernav li').click(function () {
        $('.content .shopping .retailernav li').removeClass('active');
        $('.content .shopping .retailer').css("min-height", $('.content .shopping .retailer').height());
        $('.content .shopping .retailer').load("/ajax/shopping/header?vendor=" + $(this).attr('class'));
        $('.content .shopping .retailer').attr('class', 'retailer ' + $(this).attr('class'));
...
function setupCountrySelect() {
    $('.country a').click(function (e) {
        e.preventDefault();
        $('#ajaxContainer').dialog("option", "position", { my: "right", at: "left bottom", of: $(this) });
        $('#ajaxContainer').empty().load("/ajax/master/countrySelect").dialog("open");
    });
}
...
//    //    e.preventDefault();
//    //    $('#ajaxContainer').dialog("option", "position", { my: "center", at: "center", of: window });
//    //    $('#ajaxContainer').empty().load("/ajax/master/loginForm").dialog("open");
//    //});
//}
...
//    //    e.preventDefault();
//    //    $('#ajaxContainer').dialog("option", "position", { my: "center", at: "center", of: window });
//    //    $('#ajaxContainer').empty().load("/ajax/master/signupForm").dialog("open");
...
function setupAddTagForm() {
    $(".addtagLink").click(function (e) {
        e.preventDefault();
        $('#ajaxContainer').dialog("option", "position", { my: "center", at: "center", of: window });
        $('#ajaxContainer').empty().load("/ajax/sets/" + $(this).attr('href')).dialog("open");
    });

    $(".addminifigtagLink").click(function (e) {
        e.preventDefault();
        $('#ajaxContainer').dialog("option", "position", { my: "center", at: "center", of: window });
        $('#ajaxContainer').empty().load("/ajax/minifigs/" + $(this).attr('href')).dialog("open");
    });

    $(".addparttagLink").click(function (e) {
        e.preventDefault();
        $('#ajaxContainer').dialog("option", "position", { my: "center", at: "center", of: window });
        $('#ajaxContainer').empty().load("/ajax/parts/" + $(this).attr('href')).dialog("open");
    });

}

function showMessage(selector, data) {
    var d = data.split('|', 2);
    var type = d[0];
    var text = d[1];
    if ($(selector).is(":visible")) {
        $(selector).removeClass('error').removeClass('info');
    } else {
        $(selector).hide();
    }
...
function setupOwnershipControls() {
    ...
    $('input.ownCheckbox').click(function () {
        var data = this.value.split(';', 2);
        var setID = data[0];
        var userID = data[1];
        var own = 0;
        if (userID != 0) {
            if (this.checked) own = 1;

            $.post('/ajax/collection/updateSetOwned', { 'setID': setID, 'userID': userID, 'own': own }, function (data) {
                var returnedData = data.split(';', 2);
                var setID = returnedData[0];
                var own = returnedData[1];
                if (own == 1) {
                    if (showAdditionalCollectionInput == 1) {
                        $('#owninfo' + setID).slideDown(200);
                        $('#qtynew' + setID).val(1);
                        $('#qtyused' + setID).val(0);
                    } else {
                        $('#qty' + setID).removeAttr('disabled').val(1);  
                    }
                    $('#own' + setID).addClass('itemowned');
                    $('#acm_' + setID).show();
                } else {
                    if (showAdditionalCollectionInput == 1) {
                        $('#owninfo' + setID).slideUp(200);
                        $('#qtynew' + setID).val(0)
                        $('#qtyused' + setID).val(0)
                    } else {
                        $('#qty' + setID).attr('disabled', 'disabled').val(0); 
                    }
                    $('#own' + setID).removeClass('itemowned');
                    $('#acm_' + setID).hide();
    ...
    $('input.wantCheckbox').click(function () {
        var data = this.value.split(';', 2);
        var setID = data[0];
        var userID = data[1];
        var want = 0;
        if (userID != 0) {
            if (this.checked) want = 1;
            $.post('/ajax/collection/updateSetWanted', { 'setID': setID, 'userID': userID, 'want': want }, function (data) {
                var returnedData = data.split(';', 2);
                var setID = returnedData[0];
                var want = returnedData[1];
                if (want == 1) {
                    if (showAdditionalCollectionInput == 1) {
                        $('#wantinfo' + setID).slideDown(200);
                        $('#qtywanted' + setID).val(1); 
                    }
                    $('#want' + setID).addClass('itemwanted');

                } else {
                    if (showAdditionalCollectionInput == 1) {
                        $('#wantinfo' + setID).slideUp(200);
                        $('#qtywanted' + setID).val(0);
                    }
                    $('#want' + setID).removeClass('itemwanted');
    ...
    $('input.flagCheckbox').click(function () {
        var data = this.value.split(';', 3);
        var setID = data[0];
        var userID = data[1];
        var flag = data[2];
        var value = 0;
        if (userID != 0) {
            ...
            $.post('/ajax/collection/updateFlag', { 'setID': setID, 'userID': userID, 'value': value, 'flag': flag }, function (data) {
                var returnedData = data.split(';', 3);
                var setID = returnedData[0];
                var flag = returnedData[1];
                var value = returnedData[2];

                if (value == 1) {
                    $('#' + flag + '_' + setID).addClass('item' + flag);
                } else {
                    $('#' + flag + '_' + setID).removeClass('item' + flag);
                }
    ...
    $('input.minifigFlagCheckbox').click(function () {
        var data = this.value.split(';', 3);
        var minifigNumber = data[0];
        var userID = data[1];
        var flag = data[2];
        var value = 0;
        if (userID != 0) {
            if (this.checked) value = 1;
            $.post('/ajax/collection/updateMinifigFlag', { 'minifigNumber': minifigNumber, 'userID': userID, 'value': value, 'flag': flag }, function (data) {
                var returnedData = data.split(';', 3);
                var minifigNumber = returnedData[0];
                var flag = returnedData[1];
                var value = returnedData[2];

                if (value == 1) {
                    $('#' + flag + '_' + minifigNumber).addClass('item' + flag);
                } else {
                    $('#' + flag + '_' + minifigNumber).removeClass('item' + flag);
                }
    ...
    $('input.qtyOwned').change(function () {
        var data = this.name.split(';', 2);
        var setID = data[0];
        var userID = data[1];
        if (userID != 0)
            $.post('/ajax/collection/updateSetQtyOwned', { 'setID': setID, 'userID': userID, 'qty': this.value }, function (data) {
                $('#qty' + setID).addClass('bold').val(data);
                $('#qtynew' + setID).val(data);
                $('#qtyused' + setID).val(0);
            })
        else
            alert(notLoggedIn);
    ...
    $('input.qtyWanted').change(function () {
        var data = this.name.split(';', 2);
        var setID = data[0];
        var userID = data[1];
        if (userID != 0)
            $.post('/ajax/collection/updateSetQtyWanted', { 'setID': setID, 'userID': userID, 'qty': this.value }, function (data) {
                $('#qtywanted' + setID).addClass('bold').val(data);
            })
        else
            alert(notLoggedIn);
    });

    $('input.qtyOwnedNew').change(function () {
        var data = this.name.split(';', 2);
        var setID = data[0];
        var userID = data[1];
        if (userID != 0)
            $.post('/ajax/collection/updateSetQtyOwned', { 'setID': setID, 'userID': userID, 'qty': this.value, 'which': 'new' }, function (data) {
                $('#qtynew' + setID).addClass('bold').val(data);
             //   $('#qty' + setID).val(parseInt($('#qtynew' + setID).val()) + parseInt($('#qtyused' + setID).val()));
            })
        else
            alert(notLoggedIn);
    });
    ...
    $('input.qtyOwnedUsed').change(function () {
        var data = this.name.split(';', 2);
        var setID = data[0];
        var userID = data[1];
        if (userID != 0)
            $.post('/ajax/collection/updateSetQtyOwned', { 'setID': setID, 'userID': userID, 'qty': this.value, 'which': 'used' }, function (data) {
                $('#qtyused' + setID).addClass('bold').val(data);
           //     $('#qty' + setID).val(parseInt($('#qtynew' + setID).val()) + parseInt($('#qtyused' + setID).val()));
            })
        else
            alert(notLoggedIn);
    ...
    $('select.wantedPriority').change(function () {
        var data = this.name.split(';', 2);
        var setID = data[0];
        var userID = data[1];

        if (userID != 0)
            $.post('/ajax/collection/updateSetWantedPriority', { 'setID': setID, 'userID': userID, 'priority': this.value }, function (data) {
                $('#wantedpriority' + setID).addClass('bold').value = data;
            })
        else
            alert(notLoggedIn);
    });
    ...
    $('input.mfOwnCheckbox').click(function () {
        var data = this.value.split(';', 2);
        var minifigNumber = data[0];
        var userID = data[1];
        var own = 0;
        if (userID != 0) {
            if (this.checked) own = 1;
            $.post('/ajax/collection/updateMinifigCollection', { 'minifigNumber': minifigNumber, 'userID': userID, 'own': own }, function (data) {
                var returnedData = data.split(';', 2);
                var minifigNumber = returnedData[0];
                var own = returnedData[1];
                if (own==1) {
                    $('#own' + minifigNumber).addClass('itemowned');
                    $('#qty' + minifigNumber).removeAttr('disabled').attr('value', '1');
                } else {
                    $('#own' + minifigNumber).removeClass('itemowned');
                    $('#qty' + minifigNumber).attr('disabled', 'disabled').attr('value', '0');
                }
            });
        } else
            alert(notLoggedIn);
    ...
    $('input.mfWantCheckbox').click(function () {
        var data = this.value.split(';', 2);
        var minifigNumber = data[0];
        var userID = data[1];
        var want = 0;
        if (userID != 0) {
            if (this.checked)  want = 1;
            $.post('/ajax/collection/updateMinifigCollection', { 'minifigNumber': minifigNumber, 'userID': userID, 'want': want }, function (data) {
                var returnedData = data.split(';', 2);
                var minifigNumber = returnedData[0];
                var want = returnedData[1];
                if (want==1) {
                    $('#want' + minifigNumber).addClass('itemwanted');
                } else {
                    $('#want' + minifigNumber).removeClass('itemwanted');
                }
    ...
    $('input.mfQtyOwned').change(function () {
        var data = this.name.split(';', 2);
        var minifigNumber = data[0];
        var userID = data[1];
        if (userID != 0) 
            $.post('/ajax/collection/updateMinifigQtyOwned', { 'minifigNumber': minifigNumber, 'userID': userID, 'qty': this.value }, function (data) {
                $('#qty' + minifigNumber).addClass('bold');
                $('#qty' + minifigNumber).val(data);
            })
         else
            alert(notLoggedIn);
    ...
    $('input.partOwnCheckbox').click(function () {
        var data = this.value.split(';', 2);
        var partID = data[0];
        var userID = data[1];
        var own = 0;
        if (userID != 0) {
            if (this.checked) own = 1;
            $.post('/ajax/collection/updatePartsCollection', { 'partID': partID, 'userID': userID, 'own': own }, function (data) {
                var returnedData = data.split(';', 2);
                var partNumber = returnedData[0];
                var own = returnedData[1];
                if (own == 1) {
                    $('#own' + partNumber).addClass('itemowned');
                    $('#qty' + partNumber).removeAttr('disabled').attr('value', '1');
                } else {
                    $('#own' + partNumber).removeClass('itemowned');
                    $('#qty' + partNumber).attr('disabled', 'disabled').attr('value', '0');
                }
            });
        } else
            alert(notLoggedIn);
    ...
    $('input.partWantCheckbox').click(function () {
        var data = this.value.split(';', 2);
        var partID = data[0];
        var userID = data[1];
        var want = 0;
        if (userID != 0) {
            if (this.checked) want = 1;
            $.post('/ajax/collection/updatePartsCollection', { 'partID': partID, 'userID': userID, 'want': want }, function (data) {
                var returnedData = data.split(';', 2);
                var partNumber = returnedData[0];
                var want = returnedData[1];
                if (want == 1) {
                    $('#want' + partNumber).addClass('itemwanted');
                    $('#qtyw' + partNumber).removeAttr('disabled').attr('value', '1');
                } else {
                    $('#want' + partNumber).removeClass('itemwanted');
                    $('#qtyw' + partNumber).attr('disabled', 'disabled').attr('value', '0');
                }
    ...
    $('input.partQtyOwned').change(function () {
        var data = this.name.split(';', 2);
        var partID = data[0];
        var userID = data[1];
        if (userID != 0)
            $.post('/ajax/collection/updatePartsQtyOwned', { 'partID': partID, 'userID': userID, 'qty': this.value }, function (data) {
                $('#qty' + partID).addClass('bold');
                $('#qty' + partID).val(data);
            })
        else
            alert(notLoggedIn);
    ...
    $('input.partQtyWanted').change(function () {
        var data = this.name.split(';', 2);
        var partID = data[0];
        var userID = data[1];
        if (userID != 0)
            $.post('/ajax/collection/updatePartsQtyWanted', { 'partID': partID, 'userID': userID, 'qty': this.value }, function (data) {
                $('#qtyw' + partID).addClass('bold');
                $('#qtyw' + partID).val(data);
            })
        else
            alert(notLoggedIn);
    ...
    $('input.releasedCheckbox').click(function () {
        var data = this.value.split(';', 2);
        var setID = data[0];
        var userID = data[1];
        $.post('/webhandlers/setIsReleased.ashx', { 'setID': setID, 'userID': userID }, function (data) { });
    });


    $('input.imageCheckbox').click(function () {
        var data = this.value.split(';', 2);
        var setID = data[0];
        var userID = data[1];
        $.post('/webhandlers/setHasImage.ashx', { 'setID': setID, 'userID': userID }, function (data) { });
    });


    $('input.notesSave').click(function () {
        var data = this.name.split(';', 2);
        var setID = data[0];
        var userID = data[1];

        var notes = document.getElementById('ta' + this.name).value;
        $.post('/ajax/collection/updateNotes', { 'setID': setID, 'userID': userID, 'notes': notes }, function (data) {
            $('#nt' + data).slideUp().html('<p>Your note has been saved.</p>').addClass('info').slideDown(50);
        });
        
    });

    $('input.partNotesSave').click(function () {
        var data = this.name.split(';', 2);
        var pID = data[0];
        var userID = data[1];
        var notes = document.getElementById('ta' + this.name).value;
        $.post('/ajax/parts/updateNotes', { 'pID': pID, 'userID': userID, 'notes': notes }, function (data) {
            $('#nt' + data).slideUp().html('<p>Your note has been saved.</p>').addClass('info').slideDown(50);
        });

    });

    $('input.minifigNotesSave').click(function () {
        var data = this.name.split(';', 2);
        var mID = data[0];
        var userID = data[1];

        var notes = document.getElementById('ta' + this.name).value;
        $.post('/ajax/minifigs/updateNotes', { 'mID': mID, 'userID': userID, 'notes': notes }, function (data) {
            $('#nt' + data).slideUp().html('<p>Your note has been saved.</p>').addClass('info').slideDown(50);
        });
    ...
    if ($.isFunction($.fn.editable)) {
        // https://jeditable.elabftw.net/api/
        $(".setNote").editable("/ajax/collection/updateNotes", {
            id: 'setID',
            name: 'notes',
      //      type: 'textarea',
      //      submit: 'Save',
      ...
      //      rows: 2,
            ...
            callback: function (result, settings, submitdata) {
                ...
                if (result.length-4 != 0)
                    $(ele).addClass('itemnote')
                else
                    $(ele).removeClass('itemnote');
            }
        });

        $(".minifigNote").editable("/ajax/minifigs/updateNotes", {
            id: 'mID',
            name: 'notes',
          //  type: 'textarea',
         //   submit: 'Save',
        ...
        $(".partNote").editable("/ajax/parts/updateNotes", {
            id: 'pID',
            name: 'notes',
         //   type: 'textarea',
        //    submit: 'Save',
    ...
    $('a.acmlink').click(function (e) {
        e.preventDefault;
        e.stopPropagation;
        var data = this.id.split('_')
        var setID = data[1]
        $(this).attr('href', '/ajax/acm/acm?setid=' + setID);
        hs.htmlExpand(this, { objectType: 'iframe', width: Math.min(1280, $(window).width() - 50) }) 
        return false;
    });

    $('a.bricklistlink').click(function (e) {
        e.preventDefault;
        e.stopPropagation;
        var data = this.id.split('_')
        var setID = data[1]
        $(this).attr('href', '/ajax/sets/addToBricklist?setid=' + setID);
        hs.htmlExpand(this, { objectType: 'iframe', width: Math.min(600, $(window).width() - 50) })
        return false;
    ...
    $('a.popuplink').click(function (e) {
        e.preventDefault();
        $("#ajaxContainer").dialog("option", "title", '');
        $('#ajaxContainer').dialog("option", "position", { my: 'left top', at: 'left bottom', of: $(this) });
        $('#ajaxContainer').empty().load("/ajax/sets/" + $(this).attr('href')).dialog("open");
    });
...
function setupReviewRatings() {
    $('input.yesRadio, input.noRadio').click(function () {
        var data = this.value.split(';', 3);
        ...
        if (userID != 0)
            $.get('/webhandlers/rateReview.ashx', { 'rating': rating, 'userID': userID, 'reviewID': reviewID }, function (data) { $("#confirm" + reviewID).html(data) });
        else
            alert(notLoggedIn);
    });
}

function setupSetRatings() {
    ...
    $('.setrating').barrating({
        ...
        onSelect: function (value, text, event) {
           // alert(text)
            ...
            if (userID != 0)
                $.get('/webhandlers/rateSet.ashx', { 'rating': rating, 'userID': userID, 'setID': setID }, function (data) { });
            else
                alert(notLoggedIn);
        },
    });
...
function setupBricklistRatings() {
    $('input.brickListYesRadio, input.brickListNoRadio').click(function () {
        var data = this.value.split(';', 3);
        ...
        if (userID != 0)
            $.get('/webhandlers/rateBricklist.ashx', { 'rating': rating, 'userID': userID, 'listID': listID }, function (data) { $("#confirm" + listID).html(data) });
        else
            alert(notLoggedIn);
    });
}
...
function setupVideoReviews() {
    $("#videoframe div.outer").hide();
    $("#videoframe div.outer p").hide();
    $("#videoframe div#v1").show();
    $("#meta").html(($("#mv1").html()))

    $("#publisherlist h3").click(function () {
        var state = $("#publisherlist ul#ul" + this.id).is(":visible")
        if (state == true)
            $("#publisherlist ul#ul" + this.id).slideUp();
        else {
            $("#publisherlist ul").slideUp();
            $("#publisherlist ul#ul" + this.id).slideDown();
        }
        return true;
    });

    $("#videoreviews li").click(function () {
        $("#videoframe div.outer").hide();

        var data = this.id.split(':', 3);
        var vid = data[1];
        var videoID = data[2]

        var args = new Array(videoID);
        callPlayer("iframe1", "stopVideo")
        callPlayer("iframe1", "loadVideoById", args);

        $("#" + vid).show();
        $("#meta").html(($("#m" + vid).html()))
        return true;
    });
}

function setupReviewAdmin() {
    $('body.reviews a.delreview').click(function (e) {
        e.preventDefault();
        e.stopPropagation();
        if (window.confirm("Click OK confirm you want to delete this review.")) {
            var id = right(this.id, this.id.length - 1)
            $.post('/WebHandlers/delReview.ashx', { 'reviewID': id }, function (data) {
                $("#review" + id).fadeOut();
            });
        }
        return false;
    });
    $('body.reviews a.rejectreview').click(function (e) {
        e.preventDefault();
        e.stopPropagation();
        if (window.confirm("Click OK confirm you want to reject this review.")) {
            var id = right(this.id, this.id.length - 1)
            $.post('/WebHandlers/rejectReview.ashx', { 'reviewID': id }, function (data) {
                $("#review" + id).fadeOut();
            });
        }
        return false;
...
function setupBrickListAdmin() {
    $('body.bricklists a.delbricklist').click(function (e) {
        e.preventDefault();
        e.stopPropagation();
        if (window.confirm("Click OK confirm you want to delete this BrickList.")) {
            var id = right(this.id, this.id.length - 1)
            $.post('/WebHandlers/delBrickList.ashx', { 'listID': id }, function (data) {
                $("#bricklist" + id).fadeOut();
            });
        }
        return false;
    });
...
function setupQueryAdmin() {
    $('body.queries a.delquery').click(function (e) {
        e.preventDefault();
        e.stopPropagation();
        if (window.confirm("Click OK confirm you want to delete this query.")) {
            var id = right(this.id, this.id.length - 1)
            $.post('/WebHandlers/delQuery.ashx', { 'queryID': id }, function (data) {
                $("#query" + id).fadeOut();
            });
        }
        return false;
...
//function setupPartsAdmin() {
//    $('input.partDesignCheckbox').click(function () {
//        var val = ($(this).is(":checked"))
//        var v = this.value.split(';', 2);
//        var field = v[0];
//        var designID = v[1];
//        $.post('/webhandlers/setPartDesignData.ashx', { 'designID': designID, 'value': val, 'field': field }, function (data) { });
//    });
//}

function setupSetNumberInput() {
    $("input.setnumber").autocomplete({
        source: function (request, response) {
            $.ajax({
                url: "/ajax/autocomplete/setsForEdit.aspx",
                data: "q=" + request.term,
                dataType: "json",
                type: "GET",
                contentType: "application/json; charset=utf-8",
...
function setupMinifigNameInput() {
    $("input.minifigname").autocomplete({
        source: function (request, response) {
            $.ajax({
                url: "/ajax/autocomplete/minifignames.aspx",
                data: "q=" + request.term,
                dataType: "json",
                type: "GET",
                contentType: "application/json; charset=utf-8",
...
function setupElementIDInput() {
    $("#elementid").autocomplete({
        source: function (request, response) {
            $.ajax({
                url: "/ajax/autocomplete/main",
                data: "d=elements&q=" + request.term.trim(),
                dataType: "json",
                type: "GET",
                contentType: "application/json; charset=utf-8",
                dataFilter: function (data) { return data; },
...
function addElement() {
    ...
    var currentlist = $("#elementlist").html();
    var currentlink = $("#elementsearch").attr("href");

    if (right(currentlink, 1) != "-") currentlink = currentlink + ","

    var data = $("#elementid").val().split('|');
    ...
    if (!isNaN(elementid)) { 
        ...
        $.ajax({
            type: "POST",
            url: "/ajax/parts/advancedsearchlist?elementid=" + elementid,
            contentType: false,
            processData: false,
            data: data,
            success: function (result) {
                if (result != '')
                    $("#elementlist").html(currentlist + result)
            },
        });

        $("#elementsearch").attr("href", currentlink + elementid);
        $("#elementsearch").show();
        $("#elementid").val("");
    }
}
